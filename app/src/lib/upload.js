 ...
 * expose upload progress events — XHR does, on both native and web.
 *
 * V1 has no true resume-from-offset: a dropped connection means a clean
 * retry (the whole file again), not continuing from where it stopped. Native
 * byte-range reads from a picked file need more groundwork than this pass
 * covers, so the agent's staging design leaves room for it, but the client
 * doesn't do it yet.
 */
